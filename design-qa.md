...
7. The next review pass separated current-period completeness from the response-wide 48-hour comparison status. KPI helper text and status now describe only the current 24 hours, while the chart renders whichever period has a drawable aggregate and omits any all-null series from its legend. Current-only, comparison-only, both-period, and neither-period copy remain explicit instead of presenting missing data as a successful comparison.
