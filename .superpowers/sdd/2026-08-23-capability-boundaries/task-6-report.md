...
- `evidenceSummary` 现在要求 trim 后以稳定前缀 `REDACTED:` 开头，包含非空摘要，长度不超过 512 个字符，并拒绝明显的 `data:`、`base64`、raw video/image、原始视频/图片、人脸和身份证标记。该校验是边界层轻量输入约束，不替代后续适配器的认证、授权和业务脱敏。
