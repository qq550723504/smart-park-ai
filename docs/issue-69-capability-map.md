...
| 今日重点关注 | `anomaly-overview?status=OPEN` 的高风险告警、离线设备、能耗偏差 | 仅将未处理告警计入可操作关注，同时保留设备和能耗事实；做确定性运营规则排序，不生成原因结论。规则来源与“未调用模型”说明收纳在可展开的“数据依据”中。 |
...
| 运营分析、事件与工单、运营报告、AI 助手导航 | 后续独立能力 | 本项仅登记统一导航契约；面向客户统一显示“未开放”，渲染为不可交互项，不创建空页、死按钮或技术台跳转。 |
...
- 客户界面的后端状态统一映射为准确中文；未知枚举明确显示“状态未知”，`PARTIAL` / `UNAVAILABLE` 不被成功态掩盖。
