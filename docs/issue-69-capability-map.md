...
| 人工服务请求 | `GET /api/operations/metrics` 的 `humanTicketCount` | 表示当前运行实例累计，不外推为生产规模。 |
...
| 我的待办 | `GET /api/collaboration/work-items`，只读 `CUSTOMER_AGENT` 演示视角 | 本项不审批、不更新工单；失败和空态分别展示。 |
| 最新事件 | `GET /api/operations/anomaly-evidence/{buildingId}` | 只显示后端安全摘要和时间；与当前所选楼宇及同一查询窗口绑定。 |
