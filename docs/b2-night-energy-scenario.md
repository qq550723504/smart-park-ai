...
- 参数校验直接读 fixture 的 `parameterBounds`：min/max **和 step** 都由此约束（页面输入框的 `min`/`max`/`step` 同样绑定该契约）。电价 `1.005` 虽在 `0.01—5.00` 区间内，但不满足 `0.01` 步长，会被 `validateParameters` 拒绝，无法进入确认与回执
- 清空的输入框不会被当成 0：`Number('')`/`Number(null)` 均为 `0`，但表单值先经 `toFiniteNumber()`——空白/非数字→`NaN` 直接拒绝，显式输入的 `0` 仍是合法选择（`validateParameters` 接收未解析的原始表单值，因为 HTML 数字框交回字符串）
...
- **时钟先推进、后落状态**：`commit()` 先推进虚拟时钟再执行变更，因此变更内打戳的 `createdAt`/`updatedAt`（建单 09:03、接单 09:04、应用 09:09、验证 09:00 次日）与 `virtualNow`/命令记录一致；冻结简报保留的就是这条自洽时间线
...
- URL 与模式**双向同步**：进入/翻页写入 `?scenario=b2-night-energy&scenarioPage=…`（刷新保留当前页），点击“退出演示场景”**清除**这两个参数，否则刷新会因深链仍在而立刻重新进入场景；其他无关 query 参数保留
...
- 参数表单的同步键是**run 身份 + 已应用参数值**（不含 planId）：切换方案不会丢弃尚未“应用参数”的本地修改，方案卡与确认始终基于用户正在比较的那组值；`重开本场景` 则回到 fixture 默认值
...
cd ui
npx vue-tsc -b
npx vitest run        # 56 文件 / 648 测试
npm run build
```

- `calculations.spec.ts`：派生数值与参数边界；**每个受约束参数都按 fixture 的 step 校验**（区间内的 1.005 电价 / 1.25 小时仍被拒）；**清空字段不被强转为 0**（空白/`null` 被拒，显式 0 合法）；数据不完整时估算返回 `null` 且**不发布 excess/偏差**（总览与研判均为不完整）；变体 plan delta 解析；十进制 HALF_UP 平局与联合方案月度估算
- `provider.spec.ts`：状态机、幂等、变体锁定与 plan delta 强制、失联恢复、报告幂等；pending 身份跨无关提交保留且阻塞其他变更；虚拟时钟保留 `+08:00` 偏移且工单时间戳与变更时钟一致；NO_ACTION 简报显式记录“保持观察”决定；未定稿前不写“客户明确选择”；PARTIAL 简报冻结并披露数据质量（含缺失观测 ID）且与完整简报不再同文
- `store.spec.ts`：持久化、generation 守卫、重置递增；存储受限时的空缓存降级
- `download.spec.ts`：冻结快照原样下载、无浏览器环境静默降级
- `ScenarioFlow.spec.ts`：端到端 巡检→研判→选方案→确认→接单→应用→验证→报告；PARTIAL 阻断且隐藏估算，且总览 KPI 与研判不发完整周期偏差（显示不完整），生成的简报也披露数据质量；失联重试（含刷新恢复，及 pending 期间生成报告后仍保留）；变体锁定与 NO_ACTION 方案钉住；保持观察门控；表单随 run 重置；重开导览保留 run；下载不重生成；恢复会话首帧即场景模式（不挂载在线总览）；工单页“非真实回执”标注；pending 时工单页接单禁用且可就地重试；重置需二次确认（取消保留、确认清 run）；确认后参数输入冻结且方案卡沿用回执参数；未应用的表单改动会在确认时原子保存；非法表单禁用确认；四个场景面板提供壳层 main 焦点目标；退出场景会清除深链参数（刷新不再自动进入）；切换方案保留已应用参数（不会静默回到默认值）；切换方案时保留尚未应用的本地参数修改（不会被旧草稿覆盖）；清空时长字段时确认保持禁用（不把空值当 0）；重开导览回到起点时同步把深链的 `scenarioPage` 改回 overview
