...
6. 在桌面 Demo 打开“重开导览”后，焦点进入确认框；正向/反向 Tab 均限制在取消与确认按钮之间，Escape 关闭并把焦点还给重开入口。
...
- 前端定向回归：4 个测试文件、58 个测试通过，覆盖已验收的助手状态与首页重置回归，以及本次重开确认框的焦点进入、Tab 循环、Escape 关闭和焦点返回。
