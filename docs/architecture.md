...
- 停车与能耗运营看板：以停车、能耗和空间问题卡片组织入口，点击后进入同一套只读分析流程，不在看板层缓存或编造指标数值。
