...
> 文档基线：当前分支实现。本文描述已经存在于代码库中的运行链路，不把未合并分支中的能力当作当前功能。实时语音仅在显式启用并完成预检后进入演示目录。
...
- 停车与能耗运营看板：以停车、能耗、空间、告警和设备问题卡片组织入口，点击后进入同一套只读分析流程，不在看板层缓存或编造指标数值。
...
工作流使用 Alibaba Cloud AI Graph 的 `StateGraph` 和 `MemorySaver`。内部快照包含工作流运行所需的完整状态，Web 层再映射为安全投影。告警 REST 接口的 `WorkflowResponse` 除 `workflowId`、`alertId`、`status`、`errors`、`eventSequence` 和 `riskReasons` 外，还可返回结构化的诊断、审批和工单投影：诊断保留 `id`、关联告警/设备 ID、`riskLevel`、`confidence` 和 `diagnosedAt`；审批保留 `decision` 和 `decidedAt`；工单保留工单/工作流/园区/楼宇/设备/告警 ID、`riskLevel`、`status`、创建和更新时间，以及审批决策。统一执行事件不是该 REST 投影的镜像：`GET /api/executions/{runId}/events` 只返回事件/运行 ID、序号、时间、场景、角色、阶段、事件类型、状态、`safeSummary` 和类型化 `displayPayload`；`LegacyWorkflowEventAdapter` 将告警事件映射为摘要以及工具/错误展示负载，不携带诊断风险/置信度、审批决定或工单标识/状态。诊断、审批和工单中的自由文本及证据内容在 Web DTO 中脱敏或替换为固定安全摘要。
...
- 模型只负责受约束的问题理解；通过校验的 `QueryPlan` 由服务端 `QueryPlanSqlRenderer` 生成 SQL，随后仍必须通过 SQL AST、查询计划、参数绑定和时间边界校验。这样模型不能凭空增加日期分组、过滤条件、连接或时间列。
- `EXPLAIN` 成本超限或安全策略拒绝时，最多允许一次受控修复；仍不合规则终止分析。
- 查询使用只读事务、超时、最大行数和最大结果字节数限制；查询失败不会伪造图表或结论。
- 图表单位来自指标目录而不是模型自由填写；最终总结必须能被已执行结果验证。
- 图表规格支持折线、柱状/水平排行、堆叠柱状、二维热力、日历热力、散点、KPI、仪表盘和楼宇空间分布图；表格始终是安全降级结果。
- 模型只提供图表建议，服务端会依据真实结果列校验字段、数值类型、坐标唯一性和单位一致性；模型缺省时按问题意图确定性选择可用图表。
- V3 演示迁移在白名单能耗视图中提供楼宇名称、日期/小时、面积、空间坐标、占用人数和日目标等查询维度，原始演示表仍不授予只读角色权限。
...
- **告警对外输出：** 告警 REST 接口的 Web DTO 除流程跟踪字段外，还暴露 `WorkflowResponse` 的 `status`、`diagnosis`、`approval`、`workOrder`、`errors`、`eventSequence` 和 `riskReasons` 等受控投影：诊断投影包含诊断/告警/设备 ID、风险级别、置信度和时间；审批投影包含决定和时间；工单投影包含工单/工作流/园区/建筑/设备/告警 ID、风险级别、状态、嵌套审批投影和创建/更新时间。诊断、工单和证据中的自由文本，以及审核人身份和原始审批评论按 REST 脱敏契约隐藏或替换。统一执行事件接口只暴露事件/运行 ID、序号、时间、场景、角色、阶段、事件类型、状态、`safeSummary` 和类型化展示负载；告警 legacy adapter 只映射摘要以及工具/错误负载，不暴露上述诊断风险/置信度、审批决定或工单标识/状态。客服、运营分析和专家协作接口按各自产品契约返回必要的回答、分析结果或专家发现。
- **SQL 分析：** 应用运行账号固定为 `smartpark_analytics_ro`，只授予四个白名单分析视图的 `SELECT`；管理员账号用于 Flyway 迁移、可选的本地演示数据刷新，以及应用启动时为只读角色同步运行时凭据。能耗时序 API 与 Agent 分析共享 `MetricCatalog` 和 `analytics.v_energy_hourly`，固定查询依次经过 AST、EXPLAIN 成本和只读执行门禁；稳定 DTO 不暴露视图名、SQL 或连接信息。
...
| `GET` | `/api/operations-reports/{reportId}/download` | 下载同一持久化快照对应的 PDF artifact |
...
| `GET` | `/api/operations/anomaly-overview` | 返回告警、离线设备、能耗偏差的只读异常聚合；默认近 7 天，设备带最近快照时间 |
| `GET` | `/api/operations/anomaly-evidence/{buildingId}` | 返回楼宇级告警、设备、能耗和执行轨迹安全摘要 |
...
当前实现的工作流快照、Graph checkpoint、统一执行事件、客服会话/工单、专家协作运行和反馈审计主要是进程内存储。RAG 使用进程内 `SimpleVectorStore`；运营分析的事实数据来自独立只读 PostgreSQL。运营日报是例外：它使用有界原子 JSON file store 保存结构化生成时快照、证据元数据、durable trace 和 PDF artifact，Compose 通过独立 named volume 保持重启恢复；默认最多保留 200 份、同时生成 1 份，并分别限制报告与 artifact 大小。报告读取与分析开关解耦：分析关闭时仍可查阅和下载历史，只拒绝新生成。详见 `docs/operations-reporting.md`。
