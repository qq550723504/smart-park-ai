...
function renderReportMarkdown(snapshot: ScenarioReportSnapshot): string {
  ...
  // The no-action wording must only be claimed once the decision is finalized.
  // At PLAN_SELECTED the user can still switch away from SCN-PLAN-NONE, so a
  ...
  } else if (noActionFinalized) {
    lines.push('- 客户明确选择“保持现状 / 保持观察”，本次不创建任务。')
    lines.push('- 该结论是主动决策，不是缺少可执行方案；整体偏差继续观察。')
  } else if (noActionPending) {
    lines.push('- 当前选择“保持现状 / 保持观察”，尚未确认；确认后将不创建任务。')
  } else {
