...
watch(() => props.role, () => { void load() }, { immediate: true })
...
async function load() {
  ...
  try {
    ...
    if (page.items[0]) await select(page.items[0].incidentId, generation)
  } catch (cause) {
...
<template>
  <main class="main-content security-incident-main">
    ...
    <template v-else>
      <section class="hero-row security-incident-hero"><div><span class="eyebrow">安全运营 · 06</span><h2>把零散异常<br /><em>还原成可研判事件</em></h2><p class="hero-copy">按区域、类型和时间窗口归并脱敏安全事件，保留可追溯证据并进入现有审批链。</p></div><div class="hero-metrics"><div><strong>{{ openCount || '—' }}</strong><span>待研判</span></div><div><strong>{{ highRiskCount || '—' }}</strong><span>高风险</span></div><div><strong>{{ handoffCount || '—' }}</strong><span>已转协同</span></div></div></section>
