...
export function useExpertCollaboration(pollIntervalMs = 500) {
  // The workbench uses v-show for ordinary navigation, so this instance stays
  // alive while its run is active. A deliberate workbench session reset
  // unmounts it and must not leak state into the new instance.
  ...
  async function start(question: string): Promise<string | null> {
    ...
    try {
      ...
      void poll(started.runId, currentGeneration)
