...
> **请先了解安全边界：** 本项目不是生产控制系统。Mock 适配器不会检查、切换、重启、隔离或控制真实设备；安防数据只有脱敏摘要，不包含原始视频、图片、人脸特征或身份证等人员原始记录。项目当前没有生产级认证或租户隔离；仅运营日报具备单实例、有界文件持久化，其他演示状态仍主要保存在进程内。
...
| 可恢复运营日报 | 手动汇总能耗基线、停车利用率和高风险告警三个固定只读章节；仅 OPERATOR/ADMIN 可用，保存生成时快照、证据、轨迹与 Markdown 下载，单实例重启后可恢复历史 |
...
- [持久化运营报告、历史与下载](docs/operations-reporting.md)
